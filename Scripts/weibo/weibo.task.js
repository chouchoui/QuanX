...
function GetCookie() {
  if ($request && $request.method != "OPTIONS" && $request.url.indexOf("gsid=") > -1) {
    ...
    let token = signurlVal.replace(
        /(.+)(from=\w+)(.+)(&uid=\d+)(.+)(&gsid=[_a-zA-Z0-9-]+)(&.+)(&s=\w+)(.+)/,
        "$2$4$6$8"
      ),
...
async function showmsg() {
  if (paybag) {
    $.msg($.name, signcash ? signcash : "", wbsign + paybag + docard);
    if ($.isNode()) {
      await notify.sendNotify($.name, (signcash ? signcash : "") + "\n" + wbsign + paybag + docard);
    }
  }
}
