const $ = new Env("时代天使");
$.headers = "vei_angelalign_headers";

const method = `POST`;
const headersStr = $.getdata($.headers);
...
function getPlanList() {
  const url = `https://exp.angelalign.com/api/v1/mini_program/get_plan_list`;
  const body = `{"payload":{}}`;
  const options = {
    url: url,
    method,
      ...
      try {
        if (error) {
          ...
          if (result.errmsg === "成功") {
            ...
            $.msg($.name, "获取plan list失败", result.errmsg);
            resolve([]);
          }
        }
      } catch (error) {
