...
const blackKey = "vei_bilibili_feed_black";

//Customize blacklist
let blacklist = [];
if (magicJS.read(blackKey)) {
  blacklist = JSON.parse(magicJS.read(blackKey));
} else {
  const defaultList = [
    "共青团中央",
    "广东共青团",
  ...
  magicJS.write(defaultList, blackKey);
  ...
  if (magicJS.isResponse) {
    switch (true) {
      ...
      case /^https?:\/\/api\.live\.bilibili\.com\/xlive\/app-room\/v1\/index\/getInfoByRoom/.test(
        ...
        break;
      // 追番去广告
      case /^https?:\/\/api\.bilibili\.com\/pgc\/page\/bangumi/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
  ...
  return new (class {
    constructor() {
      ...
      if (this.isNode) {
        ...
        try {
          this.node.fs.accessSync(
            "./magic.json",
            this.node.fs.constants.R_OK | this.node.fs.constants.W_OK
          );
        } catch (e) {
    ...
    adapterHttpOptions(e, t) {
      let i =
        typeof e === "object" ? Object.assign({}, e) : { url: e, headers: {} };
      ...
      if (
        (typeof i["opts"] === "object" &&
          (i["opts"]["hints"] === true ||
            i["opts"]["Skip-Scripting"] === true)) ||
        (typeof i["headers"] === "object" &&
          i["headers"]["X-Surge-Skip-Scripting"] === true)
      ) {
        o = true;
      }
      if (!o) {
      ...
      if (this.isQuanX && i.hasOwnProperty("opts"))
        delete i["opts"]["Skip-Scripting"];
      if (t === "GET" && !this.isNode && !!i.body) {
        ...
        if (
          i.url.lastIndexOf("&") + 1 != i.url.length &&
          i.url.lastIndexOf("?") + 1 != i.url.length
        )
          i.url += "&";
        i.url += e;
      ...
      if (this.isQuanX) {
        if (i.hasOwnProperty("body") && typeof i["body"] !== "string")
